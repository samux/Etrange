...
namespace soclib { namespace caba {
	 ...
	 * l'adresse RAM wb_tab[4] située autour du pixel de coordonnées pixel_center_l, 
	 * pixel_center_c.
	 */
	tmpl(void)::get_cache() {
		...
		for (;;) {
			...
			if (reset_n == false) {
				...
				//On possède une adresse d'image à lire correcte
					...
					//de remplissage de cache. On sait alors que pixel_center_c
					//et pixel_center_l sont positionnés correctement
					...
					tile_nb = tile_nb%(p_WIDTH*p_HEIGHT/(T_W*T_C));
					//Fin d'une image
			...
			if (reset_n == false) {
				...
				for (int l = 0; l<T_H; l++) {
					for (int c = 0; c<T_W; c++) 
					{
						...
						if (invimg_c[c] < (pixel_center_c - C_W/2) || invimg_c[c] > (pixel_center_c + C_W/2) || invimg_l[l] < (pixel_center_l - C_H/2) || invimg[l] > (pixel_center_l + C_H/2))
							fifo.write(255);
						//Sinon on écrit le pixel du cache correspondant
						else fifo.write(cache[invimg_l%C_H][invimg_h%C_W]); //XXX TODO CHANGE THIS, C EST PAS CA
		...
		tmpl(void)::store_tile() {
			...
			for (;;) {
				...
				if (reset_n == false)  {
					...
					//en RAM. On garde le compte de la position dans l'image
						...
						for (int l = 0; l < T_H; l++) {
							...
							master0.write_blk(	img_addr + (tile_nb/(p_WIDTH/T_W))*T_W*T_H + l*p_WIDTH + (tile_nb%(p_WIDTH/T_W) - 1)*T_W, 
										mask,
										pixel_pack,
										T_W/4); 
						}
				...
				tmpl(void) process_center(int tile_nb) {
					...
					pixel_center_c = (tile_nb -1)%tile_line*C_W + C_W/2;
					pixel_center_l = (tile_nb -1)/tile_line*C_H + C_H/2;
				}

				//TODO CHANGE THIS : pour l'instant on ne fait
				//aucun calcul sur l'image. Le centre du cache est 
				//le centre de la tuile
				tmpl(void) process_invimg(int tile_nb, int** invimg_c, int ** invimg_l) {
					int tile_line = p_WIDTH/C_W;
				...
				tmpl(void)::cache_fill(uint32_t img_addr) {
					...
					if (cache_c > (p_WIDTH-1) || cache_l > (p_HEIGHT-1)) return;
					...
					if (cache_c + C_W > (p_WIDTH - 1)) cache_im_w -= (cache_c +C_W - p_WIDTH);
					...
					for (cache_line = 0; cache_line < cache_im_h; cache_line++)  
						master0.read_blk(img_addr + ((cache_im_l+cache_line)*p_WIDTH) + cache_im_c, cache_im_w/4, (uint32_t)&cache[(cache_im_l+cache_line)%C_H][cache_im_c%C_W]); 
