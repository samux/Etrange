...
/**
 * \fn static mfixed P_1(float X, float Y, mfixed coeff[4][4])
 * \brief Compute P_1 coefficient according to the pixel (X, Y) and coeff
 *
 * \return P_1 in mfixed representation
 *
 */
static mfixed P_1(float X, float Y, mfixed coeff[4][4]);
/**
 * \fn static mfixed P_2(float X, float Y, mfixed coeff[4][4])
 * \brief Compute P_2 coefficient according to the pixel (X, Y) and coeff
 *
 * \return P_2 in mfixed representation
 *
 */
static mfixed P_2(float X, float Y, mfixed coeff[4][4]);

/**
 * \fn static mfixed P_3(float X, float Y, mfixed coeff[4][4])
 * \brief Compute P_3 coefficient according to the pixel (X, Y) and coeff
 *
 * \return P_3 in mfixed representation
 *
 */
static mfixed P_3(float X, float Y, mfixed coeff[4][4]);
...
/**
 * \fn static mfixed Q_1(float X, float Y, mfixed coeff[4][4])
 * \brief Compute Q_1 coefficient according to the pixel (X, Y) and coeff
 *
 * \return Q_1 in mfixed representation
 *
 */
static mfixed Q_1(float X, float Y, mfixed coeff[4][4]);

/**
 * \fn static mfixed Q_2(float X, float Y, mfixed coeff[4][4])
 * \brief Compute Q_2 coefficient according to the pixel (X, Y) and coeff
 *
 * \return Q_2 in mfixed representation
 *
 */
static mfixed Q_2(float X, float Y, mfixed coeff[4][4]);
...
/**
 * \fn static mfixed R_1(float X, float Y, mfixed coeff[4][4])
 * \brief Compute R_1 coefficient according to the pixel (X, Y) and coeff
 *
 * \return R_1 in mfixed representation
 *
 */
static mfixed R_1(float X, float Y, mfixed coeff[4][4]);
...
static mfixed P_1(float X, float Y, mfixed coeff[4][4])
{
  mfixed t1 = fx_mul( mf(6*(X + 2)), coeff[3][0]);
  mfixed t2 = fx_add(t1, fx_mul( mf(2*X), coeff[2][1]));
...
static mfixed P_2(float X, float Y, mfixed coeff[4][4])
{
  mfixed t1 = fx_mul( mf(3*X*X + 3*X + 1) , coeff[3][0]);
  mfixed t2 = fx_add( t1, fx_mul( mf(2*X + 1), coeff[2][0]));
  mfixed t3 = fx_add( t2, fx_mul( mf(2*X*Y + Y), coeff[2][1]));
  mfixed t4 = fx_add( t3, fx_mul( mf(Y*Y), coeff[1][2]));
  mfixed t5 = fx_add( t4, fx_mul( mf(Y), coeff[1][1]));
...
static mfixed P_3(float X, float Y, mfixed coeff[4][4])
{
  mfixed t1 = fx_mul( mf(X*X*X), coeff[3][0]);
  mfixed t2 = fx_add( t1, fx_mul( mf(Y*Y*Y), coeff[0][3]));
  mfixed t3 = fx_add( t2, fx_mul( mf(X*X*Y), coeff[2][1]));
  mfixed t4 = fx_add( t3, fx_mul( mf(X*Y*Y), coeff[1][2]));
  mfixed t5 = fx_add( t4, fx_mul( mf(X*X), coeff[2][0]));
  mfixed t6 = fx_add( t5, fx_mul( mf(X*Y), coeff[1][1]));
  mfixed t7 = fx_add( t6, fx_mul( mf(Y*Y), coeff[0][2]));
  mfixed t8 = fx_add( t7, fx_mul( mf(X), coeff[1][0]));
  mfixed t9 = fx_add( t8, fx_mul( mf(Y), coeff[0][1]));
...
static mfixed Q_1(float X, float Y, mfixed coeff[4][4])
{
  mfixed t1 = fx_mul( mf(2*X), coeff[1][2]);
  mfixed t2 = fx_add(t1, fx_mul( mf( 6*(Y+1)), coeff[0][3]));
...
static mfixed Q_2(float X, float Y, mfixed coeff[4][4])
{
  mfixed t1 = fx_mul( mf(X*X), coeff[2][1]);
  mfixed t2 = fx_add( t1, fx_mul( mf(2*X*Y + X), coeff[1][2]));
  mfixed t3 = fx_add( t2, fx_mul( mf(3*Y*Y + 3*Y + 1), coeff[0][3]));
  mfixed t4 = fx_add( t3, fx_mul( mf(X), coeff[1][1]));
  mfixed t5 = fx_add( t4, fx_mul( mf(2*Y + 1), coeff[0][2]));
...
static mfixed R_1(float X, float Y, mfixed coeff[4][4])
{
  mfixed t1 = fx_mul( mf(2*X + 1), coeff[2][1]);
  mfixed t2 = fx_add(t1, fx_mul( mf(2*Y + 1), coeff[1][2]));
...
void init_poly()
{
  ...
  for(Y = 0; Y < NB_TILE_HEIGHT; Y++)
  {
	 ...
	 for(X = 0; X < NB_TILE_WIDTH; X++)
	 {
		...
		X_p = X * TILE_HEIGHT;
		X_norm = (float) X_p / (float) WIDTH;
		Y_norm = (float) Y_p / (float) WIDTH;

		//x coefficient
		//P
		coeff_incr_array[0][Y][X].P0 = P_0(coeff_x);
		coeff_incr_array[0][Y][X].P1 = P_1(X_norm, Y_norm, coeff_x);
		coeff_incr_array[0][Y][X].P2 = P_2(X_norm, Y_norm, coeff_x);
		coeff_incr_array[0][Y][X].P3 = P_3(X_norm ,Y_norm, coeff_x);

		//Q
		coeff_incr_array[0][Y][X].Q0 = Q_0(coeff_x);
		coeff_incr_array[0][Y][X].Q1 = Q_1(X_norm, Y_norm, coeff_x);
		coeff_incr_array[0][Y][X].Q2 = Q_2(X_norm, Y_norm, coeff_x);
		coeff_incr_array[0][Y][X].Q3 = coeff_incr_array[0][Y][X].P3;


		//R
		coeff_incr_array[0][Y][X].R0 = R_0(coeff_x);
		coeff_incr_array[0][Y][X].R1 = R_1(X_norm, Y_norm, coeff_x);
		...
		ante_up_right = P_3((float)(X_p + TILE_WIDTH - 1)/(float)WIDTH, Y_norm, coeff_x);
		min = (ante_up_right.h < min.h) ? ante_up_right : min;

		ante_bottom_left = P_3(X_norm, (float)(Y_p + TILE_HEIGHT - 1)/(float)HEIGHT, coeff_x);
		min = (ante_bottom_left.h < min.h) ? ante_bottom_left : min;

		ante_bottom_right = P_3((float)(X_p + TILE_WIDTH - 1)/(float)WIDTH, (float)(Y_p + TILE_HEIGHT - 1)/(float)HEIGHT, coeff_x);
		...
		coeff_incr_array[1][Y][X].P1 = P_1(X_norm, Y_norm, coeff_y);
		coeff_incr_array[1][Y][X].P2 = P_2(X_norm, Y_norm, coeff_y);
		coeff_incr_array[1][Y][X].P3 = P_3(X_norm, Y_norm, coeff_y);

		//Q
		coeff_incr_array[1][Y][X].Q0 = Q_0(coeff_y);
		coeff_incr_array[1][Y][X].Q1 = Q_1(X_norm, Y_norm, coeff_y);
		coeff_incr_array[1][Y][X].Q2 = Q_2(X_norm, Y_norm, coeff_y);
		coeff_incr_array[1][Y][X].Q3 = coeff_incr_array[1][Y][X].P3;


		//R
		coeff_incr_array[1][Y][X].R0 = R_0(coeff_y);
		coeff_incr_array[1][Y][X].R1 = R_1(X_norm, Y_norm, coeff_y);
		...
		ante_up_right = P_3((float)(X_p + TILE_WIDTH - 1)/(float)WIDTH, Y_norm, coeff_y);
		min = (ante_up_right.h < min.h) ? ante_up_right : min;

		ante_bottom_left = P_3(X_norm, (float)(Y_p + TILE_HEIGHT - 1)/(float)HEIGHT, coeff_y);
		min = (ante_bottom_left.h < min.h) ? ante_bottom_left : min;

		ante_bottom_right = P_3((float)(X_p + TILE_WIDTH - 1)/(float)WIDTH, (float)(Y_p + TILE_HEIGHT - 1)/(float)HEIGHT, coeff_y);
		min = (ante_bottom_right.h < min.h) ? ante_bottom_right : min;
