...
On peut voir sur l'image ci-dessous un exmeple de transformation comprenant un zoom, une translation, une rotation ainsi que des termes d'ordres 2. Le résultat est plutôt satisfaisasnt même s'il reste quelques petits décalages de certaines lignes de tuiles. Toute la partie blanche de l'image correspond aux antécédents qui ne sont pas dans le cache ou des caches qui ont une intersection vide avec l'image.
\begin{figure}[!h]
	\centering
	\includegraphics[scale = 0.3]{transfo.png}
	\caption{Une transformation}
\end{figure}
